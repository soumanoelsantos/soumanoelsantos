...
import { useNonSDRSellersList } from '@/hooks/useNonSDRSellersList';

interface NonSDRSalespersonFilterProps {
  selectedSalespeople: string[];
  onSelectionChange: (salespeople: string[]) => void;
}

const NonSDRSalespersonFilter: React.FC<NonSDRSalespersonFilterProps> = ({
  selectedSalespeople,
  onSelectionChange
}) => {
  const [isOpen, setIsOpen] = React.useState(false);
  const { sellers, isLoading } = useNonSDRSellersList();

  // Criar lista de opções incluindo "Todos os Vendedores"
  const salespeople = [
...
export default NonSDRSalespersonFilter;
